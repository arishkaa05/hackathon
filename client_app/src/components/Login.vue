...
import { createBucket, getBucket } from "../hooks/useBasket";
import { useUserStore } from "../stores/userStore";
import { useBasketStore } from "../stores/basketStore";
import { useFavoriteStore } from "../stores/favoriteStore";
import { useRouter } from "vue-router";
import { Product } from "../hooks/useProduct";
import { createFavorite, getFavorite } from "../hooks/useFavorite";
...
const sendData = async () => {
  ...
  if (result.status) {
    ...
    setTimeout(async () => {
      const responseBucket = await createBucket();
      const res = await getBucket(result.data.id);
    ...
    const responseLike = await createFavorite();
    const resLike = await getFavorite(result.data.id);
